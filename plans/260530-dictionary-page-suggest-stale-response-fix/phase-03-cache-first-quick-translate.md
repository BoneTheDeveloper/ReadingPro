...
- Functional: Dictionary-scope quick translate prefers local `DictionaryEntry` rows, then `DictionaryAlias`, then safe tiny rules.
...
3. Add or reuse shared dictionary lookup helper with order: exact entry -> alias -> safe tiny rules -> deterministic fallback.
...
- [ ] Quick translate safe-rule hit works only for approved conservative rules.
