...
## Success Criteria
- [ ] CI still runs lint, typecheck, test, and coverage.
- [ ] CI runs Playwright when all required secrets are present.
- [ ] CI skips Playwright with a clear message when secrets are missing.
- [ ] E2E does not fail with a confusing auth or database error when prerequisites are absent.
