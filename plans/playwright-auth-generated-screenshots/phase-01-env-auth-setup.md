...
## Success Criteria
- [ ] `pnpm e2e --project=setup` writes `.auth/user.json` locally when credentials are present.
- [ ] Missing credentials fail with a clear, actionable message.
- [ ] No Playwright command creates a Supabase user automatically.
- [ ] CI can provide the same variables through secrets without needing `.env.test`.
