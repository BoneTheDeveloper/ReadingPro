...
- [ ] `pnpm e2e --project=authenticated` runs setup first and reuses `.auth/user.json`.
- [ ] `/en/study` smoke test fails clearly if redirected to `/en/sign-in`.
- [ ] Public and authenticated tests can still run together through `pnpm e2e`.
