...
5. Review translation cache-key and vocabulary-key behavior for normalization gaps without changing existing user data unless a migration is explicitly needed.
...
- [ ] Seed check policy is documented in `docs/database/seed-data.md`, including local/PR CI validation, read-only preview/production status checks, and approval-only mutating seed/import.
