...
- Modify: `scripts/dictionary/import-dictionary.ts`
- Modify: `scripts/dictionary/normalize-dictionary.ts`
...
2. Tighten validation for duplicate aliases, missing primary translations, unsupported language pairs, invalid source labels, and unstable ranks.
3. Preserve route-level behavior from Phase 2 while improving DTO consistency and deterministic ranking.
4. Review translation cache-key and vocabulary-key behavior for normalization gaps without changing existing user data unless a migration is explicitly needed.
5. Add benchmark fixtures for any newly stabilized dictionary or translation scenario.
6. Keep hard query budgets for suggest/lookup/entry-detail and document any justified budget changes for search or fallback translation.
7. Update dictionary and translation flow docs after tests and benchmarks define the final behavior.

## Success Criteria

- [ ] `pnpm db:validate:dictionary` passes with actionable failures when fixtures are malformed.
...
- [ ] `pnpm test:performance` passes current hard query budgets.
- [ ] Docs explain current dictionary/translation boundaries and deferred cache/search work.
