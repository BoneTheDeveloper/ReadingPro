...
The app uses `next-intl` with the Next.js App Router. User-facing pages live under the locale segment in `src/app/[locale]`, messages live in top-level JSON catalogs under `messages/`, and locale-aware navigation is centralized in `src/i18n/`.

Current locales:

| Locale | Purpose | Catalog |
|--------|---------|---------|
| `en` | Default English UI | `messages/en.json` |
| `vi` | Vietnamese UI | `messages/vi.json` |
...
| Request-time locale and messages | `src/i18n/request.ts` | Validates `requestLocale` with `hasLocale` and imports `messages/{locale}.json`. |
