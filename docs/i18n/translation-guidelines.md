...
Translations are stored as JSON catalogs in `messages/en.json` and `messages/vi.json`. English is the source language and Vietnamese mirrors the same key structure. The app uses `next-intl`, so message keys, placeholders, and rich text patterns should stay compatible with next-intl message formatting.

## Catalog Rules

- Keep `messages/en.json` and `messages/vi.json` structurally identical.
...
- The same flattened keys exist in both `messages/en.json` and `messages/vi.json`.
