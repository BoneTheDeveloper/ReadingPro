...
| Dictionary | `src/features/dictionary` | `src/app/api/dictionary` | Dictionary lookup, search, suggest, and entry services | `dictionary_*` | [Dictionary API](API/Routes/dictionary.md), [Dictionary flow](Flows/data-flows/dictionary-flow.md), [Dictionary UI](Architecture/frontend-ui-architecture/dictionary-page.md) |
...
| Auth | Clerk auth pages; layout auth controls | Clerk-hosted auth pages | Auth sync and ownership helpers; `src/proxy.ts` | `profiles` | [Auth architecture](Architecture/auth-architecture.md), [Auth flow](Flows/data-flows/auth-flow.md), [Auth UI](Architecture/frontend-ui-architecture/auth-pages.md) |
