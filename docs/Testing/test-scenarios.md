...
| TS-05 | a passage with original + simplified content | the reader renders | line-numbered text and a working simplified toggle are shown | US-03 / UC-02 | `src/contracts/reading-utils.test.ts`, `tests/vitest/integration/services/passage-study-service.test.ts` |
...
| TS-15 | a selection exceeding limits or unowned passage | translate is called | the request is rejected | US-10 / UC-09 | `src/contracts/translation/translation-limits.test.ts`, `src/server/modules/translation/quick-selection-scope.test.ts` |
