  ...
  return (
    <button
      ...
      className={cn(
        ...
        showFeedback && isCorrectOption && "border-success bg-success-soft",
        showFeedback && isSelected && !isCorrectOption && "border-danger bg-danger-soft",
    ...
    >
      <span className={cn(
        ...
        showFeedback && isCorrectOption && "border-success bg-success text-primary-foreground",
        showFeedback && isSelected && !isCorrectOption && "border-danger bg-danger text-primary-foreground",
      )}>
        {optionId}
      </span>
      <span className="flex-1 text-foreground">{optionText}</span>
      {showFeedback && isCorrectOption && <CheckCircle className="w-5 h-5 text-success shrink-0" />}
      {showFeedback && isSelected && !isCorrectOption && <XCircle className="w-5 h-5 text-danger shrink-0" />}
    </button>
  )
}
...
function AnswerFeedback({ isCorrect, explanation }: { isCorrect: boolean; explanation: string }) {
  ...
  return (
    <div className={cn("p-4 rounded-xl", isCorrect ? "bg-success-soft border border-success/40" : "bg-danger-soft border border-danger/40")}>
      <div className={cn("flex items-center gap-2 mb-2 font-semibold", isCorrect ? "text-success" : "text-danger")}>
