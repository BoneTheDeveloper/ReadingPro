...
export function QuestionResults({
  correctCount, totalQuestions, passageTitle, artifactId,
  onReset, onNewPassage, onRecordResult, onResetResult,
  ...
  const recordResult = useCallback(async () => {
    ...
    try {
      ...
      setSaveError(err instanceof Error ? err.message : "Lưu tiến trình thất bại");
    } finally {
      isSavingRef.current = false;
    }
  }, [artifactId, correctCount, totalQuestions, onRecordResult]);
      ...
      {saveError && (
        <div className="absolute top-0 left-0 right-0 bg-danger-soft/90 border-b border-danger/20 p-2.5 px-4 flex items-center justify-between z-10 animate-in slide-in-from-top-2">
          <div className="flex items-center gap-2 text-danger text-sm font-medium">
            ...
            <span>Lưu tiến trình thất bại</span>
          </div>
