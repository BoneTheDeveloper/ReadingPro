...
import { getCEFRColor, getCEFRLabel } from '@/lib/cefr-utils';
...
export function StudyLeftPanel({ status, passage, error, onAnalyze }: StudyLeftPanelProps) {
  ...
  if (status === 'ready' && passage) {
    const currentContent = viewMode === 'simplified' && passage.simplifiedContent
      ? passage.simplifiedContent
      : passage.content;
    const currentLevel = viewMode === 'simplified' ? passage.simplifiedLevel : passage.originalLevel;
    const level = (currentLevel || passage.originalLevel || 'B2') as Parameters<typeof getCEFRColor>[0];
    const readingTime = calculateReadingTime(passage.wordCount, level);

    return (
      <div className="p-6">
        <div className="bg-white rounded-2xl p-6 shadow-sm">
          <h2 className="text-xl font-bold text-neutral-900 mb-4">{passage.title}</h2>
            ...
            <div className="flex items-center gap-2 mb-4 p-1 bg-neutral-100 rounded-lg">
              <button
                ...
                className={cn(
                  ...
                  viewMode === 'original' ? 'bg-white text-neutral-900 shadow-sm' : 'text-neutral-600 hover:text-neutral-900'
                )}
              >
              ...
              <button
                ...
                className={cn(
                  'px-3 py-1.5 rounded-md text-sm font-medium transition-colors',
                  viewMode === 'simplified' ? 'bg-white text-neutral-900 shadow-sm' : 'text-neutral-600 hover:text-neutral-900'
                )}
              >
      ...
      <div className="flex gap-3 mb-6">
        <button
          ...
          className={cn(
            ...
            uploadMethod === 'file' ? 'bg-primary-600 text-white' : 'bg-white text-neutral-600 border border-neutral-200 hover:bg-neutral-50'
          )}
        >
          Upload File
        </button>
        <button
          ...
          className={cn(
            'px-4 py-2 rounded-lg text-sm font-medium transition-all',
            uploadMethod === 'text' ? 'bg-primary-600 text-white' : 'bg-white text-neutral-600 border border-neutral-200 hover:bg-neutral-50'
          )}
        >
          Paste Text
        </button>
