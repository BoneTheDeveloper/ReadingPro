...
## 2025-02-28 - [Host Header Injection in Middleware]
**Vulnerability:** The Next.js middleware (`src/proxy.ts`) relied on `request.url` when constructing redirect URLs for unauthenticated users (e.g., `redirect_url` query parameters or auth page redirects). An attacker could manipulate the HTTP `Host` header, causing the middleware to generate a redirect URL pointing to an external malicious domain, potentially resulting in an Open Redirect and phishing attacks.
**Learning:** In Next.js middleware, `request.url` and `request.nextUrl` derive their origin from the incoming `Host` header, which is unvalidated and can be easily spoofed by attackers. These should never be trusted as the base for absolute redirects.
**Prevention:** Always use a canonical, statically defined base URL (like `process.env.NEXT_PUBLIC_SITE_URL`) when constructing absolute redirect URLs in middleware, falling back to the dynamic request URL only when absolutely necessary and carefully validated.
