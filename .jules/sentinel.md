...
## 2024-05-24 - [Path Traversal in Blob Storage Adapter]
**Vulnerability:** The local development blob storage adapter (`src/lib/storage/blob-storage.ts`) used POSIX path normalization (`path.posix.normalize`) which fails to handle Windows-style path traversal payloads (e.g. backslashes `\` and drive letters `C:`).
**Learning:** POSIX normalization functions ignore Windows-specific path separators and absolute path indicators, leaving a blind spot for path traversal when parsing unsanitized input that might originate from different operating systems.
**Prevention:** Always strip absolute path indicators (like drive letters) and convert all slashes to a uniform direction (e.g. forward slashes `/`) *before* applying path normalization algorithms, especially when explicitly enforcing POSIX rules on potentially mixed input.
