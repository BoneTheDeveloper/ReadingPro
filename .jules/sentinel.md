...
## 2025-02-15 - Security Hardening: Prevented Host Header Injection in Next.js Middleware Redirects
**Vulnerability:** The `src/proxy.ts` middleware constructed absolute redirect URLs using the incoming `request.url` combined with a relative path. Because the origin in `request.url` inside Next.js middleware is constructed using the `Host` (or `X-Forwarded-Host`) header provided by the client, an attacker could supply a malicious `Host` header. This caused the middleware to generate a redirect URL pointing to an attacker-controlled external domain, resulting in an Open Redirect / Host Header Injection vulnerability.
**Learning:** Using `new URL(..., request.url)` (or even `request.nextUrl.clone()` where the origin goes unvalidated) in Next.js middleware is dangerous when constructing absolute redirects because it inherently trusts the incoming host header.
**Prevention:** Always use a statically defined environment variable, such as `process.env.NEXT_PUBLIC_SITE_URL`, as the base origin when generating absolute redirect URLs within Next.js middleware. Only fall back to `request.url` if strictly necessary.
